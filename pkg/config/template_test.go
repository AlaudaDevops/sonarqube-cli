...
func TestApplyTemplate(t *testing.T) {
	...
	t.Run("success for valid variables", func(t *testing.T) {
		...
		if res.PluginName != "tekton" {
			t.Errorf("ApplyTemplate() plugin_name = %v, want tekton", res.PluginName)
		}
...
func (failingTemplateData) Fail() (string, error) {
	return "", errors.New("render failed")
}

func TestRenderTemplate_DoesNotLeavePartialOutputOnExecuteError(t *testing.T) {
	dir := t.TempDir()
	templatePath := filepath.Join(dir, "template.tmpl")
	outputPath := filepath.Join(dir, "output.txt")

	if err := os.WriteFile(templatePath, []byte("before {{.Fail}} after"), 0600); err != nil {
		t.Fatalf("WriteFile(template) error = %v", err)
	}

	err := RenderTemplate(templatePath, outputPath, failingTemplateData{})
	if err == nil {
		t.Fatal("RenderTemplate() error = nil, want render failure")
	}
	if _, statErr := os.Stat(outputPath); !errors.Is(statErr, os.ErrNotExist) {
		t.Fatalf("output file exists after render failure: %v", statErr)
	}
}
