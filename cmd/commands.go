...
func init() {
	...
	for _, cmd := range []*cobra.Command{projectDeleteCmd, userDeleteCmd, groupDeleteCmd, tokenRevokeCmd} {
		cmd.Flags().StringVar(&endpoint, "endpoint", "", "SonarQube endpoint (fallback to SONARQUBE_URL)")
		cmd.Flags().StringVar(&token, "token", "", "Manager token (fallback to SONARQUBE_MANAGER_TOKEN)")
	}
...
func getEndpointAndToken() (string, string, error) {
	...
	t := token
	if t == "" {
		t = os.Getenv("SONARQUBE_MANAGER_TOKEN")
	}
	if t == "" {
		return "", "", fmt.Errorf("--token or SONARQUBE_MANAGER_TOKEN is required")
	}
